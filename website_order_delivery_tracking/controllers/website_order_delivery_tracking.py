...
class Tracking(http.Controller):
    ...
    def input_data_processing(self, **post):
        ...
        tracking_email = post.get('tracking_email')
        track_data = request.env['stock.picking'].sudo().search(
            ['|',('carrier_tracking_ref', '=', post.get('input_data')),('name', '=', post.get('input_data'))])

        if track_data and (track_data[0].partner_id.email == tracking_email or track_data[0].sale_id.partner_id.email == tracking_email):
            pass
        else:
            return []
